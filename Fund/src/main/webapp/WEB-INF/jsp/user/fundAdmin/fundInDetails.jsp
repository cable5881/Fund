	...
	<div class="main ">
		...
		<div class="row">
								...
								<tbody>
									<c:forEach items="${inbillList }" var="bill">
										<tr>
											...
											<td>
												<button class="btn btn-danger fa fa-trash-o " title="删除" onclick="deleteInbill(${bill.inbillId },${bill.moneyIn },${mCheckId})"></button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
			...
			<div class="row myPage">
				<imaw:page url="${pageContext.request.contextPath }/fundAdmin/checkInBill.action?mCheckId=${mCheckId}" />
			</div>
		</div>
	</div>
	
	<script type="text/javascript">
		function deleteInbill(inbillId,moneyIn,mCheckId){
			var flag = confirm("您确定要删除此到款记录吗");
			if(flag){
				$.ajax({
					type: "POST",
		             url: "${pageContext.request.contextPath}/fundAdmin/deleteInbill.action",
		             data: 'inbillId=' + inbillId + '&moneyIn=' + moneyIn + '&mCheckId=' + mCheckId,
		             dataType: "json",
