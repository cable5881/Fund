	...
	<div class="main ">
		...
		<div class="page-header">
			<div class="pull-left">
				<ol class="breadcrumb visible-sm visible-md visible-lg">
					...
					<li class="active"><a href="#"><i class="fa fa-table"></i>立项申请表</a></li>
				</ol>
			</div>
		...
		<div class="row col-md-6 form-horizontal">
			<div class="form-group">
				...
				<div class="col-md-6">
					<input type="text" id="projectNum" name="projectNum" value="${p.projectNum }" class="form-control" style="border-color: black;">
				</div>
			</div>
		</div>
		...
		<div class="col-md-6">
			<div class="col-md-4 col-md-offset-3" style="text-align: center">
				<button type='button' class="btn btn-danger"
					onclick="examineProject(3,${p.projectId})">
					<i class="fa fa-times"></i> 审核无效
				</button>
			</div>
			<div class="col-md-4 " style="text-align: center">
				<button type='button' class="btn btn-success"
					onclick="examineProject(2,${p.projectId})">
					<i class="fa fa-check-circle"></i> 审核通过
				</button>
			</div>
		</div>
	</div>
...
function examineProject(mode,projectId){
	var projectNum = $('#projectNum').val();
	var flag = false;
	if(mode==2)
	{
		flag = confirm("您确定要通过此立项申请表?");
	}
	else if(mode==3)
	{
		flag = confirm("您确定要否决此立项申请表?");
	}
	
	if(flag){
		$.ajax({
             type: "POST",
             url: "${pageContext.request.contextPath}/proExpand/examineProject.action",
             data: 'isPassed=' + mode + "&projectId=" + projectId + "&projectNum=" + projectNum,
             dataType: "json",
